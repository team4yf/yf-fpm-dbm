...
function executeSql(adapter,action,args,cb){
    ...
    if(sql === false){
      ...
      adapter.getConnection(function(err1, connection) {
        if(err1){
          cb(E.CONNECT_ERROR);
          return false;
        }else{
...
module.exports = function(adapter,options) {
  _options = _.extend(_options,options) ;
    ...
    var count = function (args, cb) {
      executeSql(adapter,'count', args, function(error, data){
        if(error){ cb(error); return; }
        cb(null, data[0]['c']);
      });
    };
    var find = function (args, cb) {
      executeSql(adapter,'select', args ,function(error, data){
        if(error){ cb(error); return; }
        cb(null, data);
      });
    };
    return {
        ...
        first:function (args, cb) {
          executeSql(adapter,'first', args, function (error, list) {
            if(error){ cb(error); return; }
            if (list.length >= 1) {
              cb(null, list[0]);
              return;
        ...
        create:function (args, cb) {
          executeSql(adapter,'insert', args, function(error, newRow){
            if(error){ cb(error); return; }
        ...
        update:function (args, cb) {
            executeSql(adapter,'update', args, function(error, data){
              if(error){ cb(error); return; }
        ...
        get:function (args, cb) {
          executeSql(adapter,'get', args, function (error, list) {
            if(error){ cb(error); return; }
            if (list.length === 1) {
              cb(null, list[0]);
        ...
        remove:function (args, cb) {
          executeSql(adapter,'remove', args, cb);
        } ,
        //删除若干数据
        clear:function (args, cb) {
          executeSql(adapter,'clear', args, cb);
        }
    };
};
